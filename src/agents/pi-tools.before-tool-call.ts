...
const executionMetadataByToolCallId = new Map<string, Record<string, unknown>>();
...
export async function runBeforeToolCallHook(args: {
  ...
  let hookResult: Awaited<ReturnType<typeof hookRunner.runBeforeToolCall>> | undefined;
  try {
      ...
      try {
        ...
        if (
          ...
          return {
            ...
            executionMetadata: hookResult?.executionMetadata,
          };
        }
  ...
  return { blocked: false, params, executionMetadata: hookResult?.executionMetadata };
}
  ...
  const wrappedTool: AnyAgentTool = {
    ...
    execute: async (toolCallId, params, signal, onUpdate) => {
      ...
      if (toolCallId) {
        ...
        if (outcome.executionMetadata) {
          executionMetadataByToolCallId.set(adjustedParamsKey, outcome.executionMetadata);
          if (executionMetadataByToolCallId.size > MAX_TRACKED_ADJUSTED_PARAMS) {
            const oldest = executionMetadataByToolCallId.keys().next().value;
            if (oldest) {
              executionMetadataByToolCallId.delete(oldest);
            }
          }
        }
      }
...
export function consumeExecutionMetadataForToolCall(
  toolCallId: string,
  runId?: string,
): Record<string, unknown> | undefined {
  const key = buildAdjustedParamsKey({ runId, toolCallId });
  const metadata = executionMetadataByToolCallId.get(key);
  executionMetadataByToolCallId.delete(key);
  return metadata;
}
